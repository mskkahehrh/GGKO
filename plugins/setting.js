...
async (conn, mek, m, { isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        const sessNum = sessionNumber || botNumber || 'global'
        await updfb(sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("WORK_TYPE", val, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("ALLWAYS_OFFLINE", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_TYPING", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_VIEW_STATUS", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_LIKE_STATUS", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_RECORDING", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_REPLY", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        const sessNum = sessionNumber || botNumber || 'global'
        await input("AUTO_AI", dbVal, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("BUTTON", val, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("OWNER_REACT", val, sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        ...
        const sessNum = sessionNumber || botNumber || 'global'
        await input("WORK_TYPE", q.trim().toLowerCase(), sessNum)
        const updated = await updb(sessNum)
        if (cfg) Object.assign(cfg, updated)
