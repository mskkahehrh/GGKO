...
const { input, get, updb, updfb, getConfig } = require("../lib/database")
...
async (conn, mek, m, { from, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const cfg = getConfig(botNumber)
        const prefix = cfg.PREFIX || config.PREFIX || '.'

        const workType   = (cfg.WORK_TYPE        || 'private').toLowerCase()
        const autoView   = (cfg.AUTO_VIEW_STATUS  || 'false').toLowerCase()
        const autoLike   = (cfg.AUTO_LIKE_STATUS  || 'false').toLowerCase()
        const autoRec    = (cfg.AUTO_RECORDING    || 'false').toLowerCase()
        const autoReply  = (cfg.AUTO_REPLY        || 'false').toLowerCase()
        const autoAI     = (cfg.AUTO_AI           || 'false').toLowerCase()
        const autoTyping = (cfg.AUTO_TYPING       || 'false').toLowerCase()
        const ownerReact = (cfg.OWNER_REACT       || 'true').toLowerCase()
        const buttonMode = (cfg.BUTTON            || 'true').toLowerCase()
        const alwaysOff  = (cfg.ALLWAYS_OFFLINE   || 'false').toLowerCase()
        const lang       = (cfg.LANG              || 'EN').toUpperCase()
...
// =========================================================
// RESET DATABASE — resets only the calling user's settings
// =========================================================
cmd({
    pattern: "resetdb",
...
async (conn, mek, m, { isOwner, senderNumber, botNumber, reply }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        await updfb(botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("WORK_TYPE", val, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("ALLWAYS_OFFLINE", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("AUTO_TYPING", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await input("AUTO_VIEW_STATUS", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await input("AUTO_LIKE_STATUS", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await input("AUTO_RECORDING", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await input("AUTO_REPLY", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await input("AUTO_AI", dbVal, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("BUTTON", val, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("OWNER_REACT", val, botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        ...
        await input("WORK_TYPE", q.trim().toLowerCase(), botNumber)
        await updb(botNumber)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        await input("PREFIX", q, botNumber)
        await updb(botNumber)
        reply(`*✅ New Prefix:* ${q}`)
    } catch (e) {
        console.log(e)
