...
const { connectdb, updb, getSessionConfig, initializeSettings } = require('./lib/database');
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // ✅ Load per-session config (User A's settings don't affect User B)
      const sessionKey = botNumber.replace(/[^0-9]/g, '');
      const sessionCfg = await getSessionConfig(sessionKey);

      // =============================
      // 🔒 WORK TYPE FILTER (per-session — each bot has its own setting)
      // =============================
      const workType = (sessionCfg.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    // ✅ Initialize per-session settings in DB (creates if not exists)
    initializeSettings(sanitizedNumber).catch(e => console.error("initializeSettings error:", e));
