...
const { connectdb, updb, sessionConfigs, getOrInitSessionConfig } = require('./lib/database');
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // Use per-session config if available, otherwise fall back to global config.
      const sessionCfg = sessionConfigs.get(number.replace(/[^0-9]/g, '')) || config;
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          // Load (or create) this session's own settings from the DB.
          await getOrInitSessionConfig(sanitizedNumber);
          console.log(`✅ Per-session config loaded for ${sanitizedNumber}`);
