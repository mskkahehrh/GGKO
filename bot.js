...
const { mergeSessionConfig, getSessionConfig, getSessionConfigSnapshot } = require('./lib/database');
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // Uses per-session config so each connected number has its own work type
      // =============================
      const sessionCfg = getSessionConfig(number.replace(/[^0-9]/g, ''));
      const workType = (sessionCfg.WORK_TYPE || config.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
      ...
      const prefix = sessionCfg.PREFIX || config.PREFIX;
      ...
      await cmdData.function(conn, mek, null, {
        ...
        reply,
        config,
        // Per-session support: the bot's own number for this socket
        sessionNumber: number.replace(/[^0-9]/g, ''),
        // Helpers so plugins can persist per-session settings to Mongo
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          // Merge saved per-session config into the in-memory session store
          // so that setting.js commands read/write the right session's config
          mergeSessionConfig(sanitizedNumber, userConfig);
